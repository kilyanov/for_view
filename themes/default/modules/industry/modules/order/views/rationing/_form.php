...
<div class="row">
    <div class="col-md-4">
        <?= $form
            ->field($model, 'unitId')
            ->dropDownList(Unit::asDropDown(['parent' => true]), ['id' => 'unitId']) ?>
    </div>
    <div class="col-md-4">
        <?= $form
    ...
    <div class="col-md-6">
        <?= $form
            ...
            ->widget(
                ...
                [
                    'model' => $model,
                    'attribute' => 'rationingId',
                    'language' => 'ru',
                    'data' => [],
                    'options' => ['placeholder' => 'Выберите нормировку...'],
                    'pluginOptions' => [
                        'dropdownParent' =>  '#' . AnswerInterface::MODAL_ID,
                        'minimumInputLength' => 1,
                        'allowClear' => true,
                        'ajax' => [
                            'url' => Url::to(['/rationing/product/list']),
                            'dataType' => 'json',
                            'data' => new JsExpression('function(params) {
                                var unitId = $(\'#unitId\').val();
                                var productId = $(\'#product-id\').val();
                                var productNodeId = $(\'#product-node-id\').val();
                                var productBlockId = $(\'#product-block-id\').val();
                                var impactId = $(\'#impactId\').val();
                                return {
                                    name:params.term,
                                    unitId:unitId,
                                    impactId:impactId,
