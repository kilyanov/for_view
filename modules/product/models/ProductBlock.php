 ...
 */
class ProductBlock extends ActiveRecord implements HiddenAttributeInterface
{
    ...
    public function rules(): array
    {
        return [
            ...
            [['description', 'mark', 'productNodeId'], 'default', 'value' => null],
    ...
    /**
     * {@inheritdoc}
     * @return ProductBlockQuery the active query used by this AR class.
     */
    public static function find(): ProductBlockQuery
    {
        return new ProductBlockQuery(get_called_class());
    }
    ...
    {
        $product = !empty($this->productRelation) ?
            'изд. ' . $this->productRelation->getFullName() : null;
        $productNode = !empty($this->productNodeRelation) ?
            $this->productNodeRelation->getFullName() : null;

        return implode(' ', array_filter([
            $this->name, $this->mark, $productNode, $product
        ]));
    }
    ...
    public static function asDropDown(array $config = []): array
    {
        ...
        $query->andFilterWhere([self::tableName() . '.[[productId]]' => ArrayHelper::getValue($config, 'productId')])
            ->andFilterWhere([self::tableName() . '.[[productNodeId]]' => ArrayHelper::getValue($config, 'productNodeId')]);
